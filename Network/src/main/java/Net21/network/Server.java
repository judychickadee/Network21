...
public class Server {
        ...
        gameThread = new Thread(() -> {
            try {
                for (int i = 0; i < 5; i++) {
                    ...
                    if (!gameStarted) {
                        break;  // Exit early if game was reset
                    }
                }
                endGame();
                ...
                gameStarted = false;  // Ensure flag is reset
            }
        });
    ...
    public static synchronized void resetGame() {
        gameStarted = false;  // Signal the game thread to stop
        if (gameThread != null && gameThread.isAlive()) {
            gameThread.interrupt();  // Interrupt if it's still running
        }
        gameThread = null;  // Clear the reference
        roomTimer = null;   // Clear the waiting room timer
        // Reset any other game state if needed (e.g., player hands, scores)
    }
